...
new_state_dict = {}
for key in sft_model.state_dict():
    if "adapter_rs" in key:
        if script_args.dpo_model_3_name is not None and script_args.weight_3 > 0:
            new_state_dict[key] = script_args.weight_1 * sft_model.state_dict()[key] + script_args.weight_2 * sft_model.state_dict()[key.replace("adapter_rs", "adapter_aux_1")] + script_args.weight_3 * sft_model.state_dict()[key.replace("adapter_rs", "adapter_aux_2")]
        else:
            new_state_dict[key] = script_args.weight_1 * sft_model.state_dict()[key] + script_args.weight_2 * sft_model.state_dict()[key.replace("adapter_rs", "adapter_aux_1")]
...
for _ in tqdm(range(iters)):
    ...
    policy_output = sft_model.generate(
...
file_path = "results{}/outputs/{}rs_output_{}_{}_{}.txt".format(dataset_prefix, output_prefix, script_args.weight_1, script_args.weight_2, script_args.f_type)
